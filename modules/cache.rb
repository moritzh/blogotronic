...
module Sinatra 
  ...
  module Cache
    ...
    module Helpers 
      ...
      def cache_expire(path)
        return unless options.cache_enabled
        
        if File.exist?("#{options.public}#{path}")
          Dir.delete("#{options.public}#{path}")
          log("Expired Page deleted at: [#{options.public}#{path}]",:info)
        else
          log("No Expired Page was found at the path: [#{options.public}#{path}]",:info)
        end
      end
